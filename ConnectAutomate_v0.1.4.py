...
class BookmarkForm:
    ...
    def create_form_2(self, state):
        ...
        for index, value in enumerate(self.fields):
            if value.tkLabel == None:
                ...
                entry.grid(row=index , column=1, pady=(5, 5))
    ...
    def fill_word_with_values(self, references, values, path, pathToSave, wordNameToSave, subfolderName, genericPrefix):
        ...
        if genericPrefixField != None:
            genericPrefixStr = f"{genericPrefixField.tkInput.get()}"
        
        if subfolderName != "":
            if subfolderName == "rien":
                subfolderName = f"{genericPrefixStr}"
            else:
    ...
    def set_list_of_value(self, id, idList=[]):
        ...
        if idList:
            valeurdeSaut = self.find_first_id_gap(0, field.id)
        if valeurdeSaut and field.id > valeurdeSaut and idList: #ici si on est dans le cas où on est plus loin que les 10eme ID et qu'on est en train de load un formulaire jsp pk mais la combobox vient se mettre 1 id trop loin.
            combobox_main.grid(row=field.id -1, column=1)
        else:
    ...
    def find_first_id_gap(self, start_id, end_id):
        """Trouve le premier ID manquant entre start_id et end_id."""
        for id in range(start_id, end_id + 1):
            if not self.find_field_with_id(id):
                return id  # Retourne le premier ID manquant
        return None  # Si aucun ID manquant, retourne None
